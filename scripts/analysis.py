...
FAVORITE_WIN_RATE_OVERALL = FAVORITE_CTE + """
SELECT
...
FAVORITE_WIN_RATE_BY_HOUR = FAVORITE_CTE + """
SELECT
...
FAVORITE_WIN_RATE_BY_WEEKDAY = FAVORITE_CTE + """
SELECT
...
JOIN match_odds mo ON mo.match_id = m.match_id
WHERE
    (m.result = 'A' AND mo.avg_away_odds >= 5.0)
    OR (m.result = 'H' AND mo.avg_home_odds >= 5.0)
ORDER BY
...
def referee_impact_for_team(conn: sqlite3.Connection, team_name: str, min_matches: int = 10) -> pd.DataFrame:
    query = """
    SELECT
        m.referee,
        COUNT(*) AS matches,
...
def run_query(conn: sqlite3.Connection, title: str, query: str) -> None:
    print(f"\n=== {title} ===")
...
def main() -> None:
    ...
    run_query(conn, "Favorite win rate (overall)", FAVORITE_WIN_RATE_OVERALL)
    run_query(conn, "Favorite win rate by kickoff hour", FAVORITE_WIN_RATE_BY_HOUR)
    run_query(conn, "Favorite win rate by weekday", FAVORITE_WIN_RATE_BY_WEEKDAY)
    run_query(conn, "Referee stats (min 30 matches)", REFEREE_STATS)
    run_query(conn, "Biggest upsets (odds >= 5.0)", BIGGEST_UPSETS)
